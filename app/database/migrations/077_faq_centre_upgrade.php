...
class Migration_077_FAQCentreUpgrade
{
    public function up(\PDO $pdo): void
    {
        ...
        $this->column($pdo, 'faq_items', 'category_id', "INT UNSIGNED NULL AFTER id");
        $this->column($pdo, 'faq_items', 'slug', "VARCHAR(180) NULL AFTER category_id");
        $this->column($pdo, 'faq_items', 'is_popular', "TINYINT(1) NOT NULL DEFAULT 0 AFTER sort_order");
        $this->column($pdo, 'faq_items', 'status', "ENUM('published','draft') NOT NULL DEFAULT 'published' AFTER is_popular");
        $this->column($pdo, 'faq_items', 'search_keywords', "TEXT NULL AFTER status");
        $this->column($pdo, 'faq_items', 'created_by', "INT UNSIGNED NULL AFTER search_keywords");
        $this->column($pdo, 'faq_items', 'updated_by', "INT UNSIGNED NULL AFTER created_by");
        $this->column($pdo, 'faq_items', 'created_at', "TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP AFTER updated_by");
        $this->column($pdo, 'faq_items', 'updated_at', "TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP AFTER created_at");
        ...
        $rows = $pdo->query("SELECT id, category FROM faq_items WHERE category_id IS NULL")->fetchAll(\PDO::FETCH_ASSOC);
        $categoryLookup = [];
        foreach ($pdo->query("SELECT id, slug, name FROM faq_categories")->fetchAll(\PDO::FETCH_ASSOC) as $category) {
            $categoryLookup[strtolower((string)$category['slug'])] = (int)$category['id'];
            $categoryLookup[strtolower((string)$category['name'])] = (int)$category['id'];
        }
        $fallback = $categoryLookup['eligibility'] ?? null;
        $update = $pdo->prepare("UPDATE faq_items SET category_id = ?, status = CASE WHEN is_visible = 1 THEN 'published' ELSE 'draft' END WHERE id = ?");
        foreach ($rows as $row) {
            $key = strtolower(trim((string)($row['category'] ?? '')));
            $update->execute([$categoryLookup[$key] ?? $fallback, (int)$row['id']]);
        }
    ...
    public function down(\PDO $pdo): void
    {
        ...
        $pdo->prepare("DELETE FROM cms_sections WHERE page_id = ? AND section_key IN ('faq_hero','faq_popular','faq_library','faq_contact_cta')")->execute([$pageId]);
    }
    ...
    private function pageId(\PDO $pdo): int
    {
        ...
        $stmt = $pdo->query("SELECT id FROM cms_pages WHERE slug = 'faq' LIMIT 1");
