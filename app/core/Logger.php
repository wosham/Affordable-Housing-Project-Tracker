...
class Logger
{
    ...
    {
        $action = substr(trim($action) !== '' ? trim($action) : 'event', 0, 100);
        $module = substr(trim($module) !== '' ? trim($module) : 'system', 0, 80);
        $ip = substr((string)($_SERVER['REMOTE_ADDR'] ?? ''), 0, 45) ?: null;
        $userAgent = substr((string)($_SERVER['HTTP_USER_AGENT'] ?? ''), 0, 255) ?: null;
        $route = substr((string)($_SERVER['REQUEST_URI'] ?? ($_SERVER['SCRIPT_NAME'] ?? 'cli')), 0, 255) ?: null;
        $method = substr((string)($_SERVER['REQUEST_METHOD'] ?? (PHP_SAPI === 'cli' ? 'CLI' : '')), 0, 10) ?: null;
        $actorRole = substr((string)(Auth::role() ?? ''), 0, 80) ?: null;
        $severity = self::severityFor($action, $module);
        $detailsJson = $details === [] ? null : json_encode($details, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        $metadata = [
            'session' => session_status() === PHP_SESSION_ACTIVE ? session_id() : null,
            'referer' => substr((string)($_SERVER['HTTP_REFERER'] ?? ''), 0, 500) ?: null,
        ];
        $eventHash = hash('sha256', implode('|', [(string)(Auth::id() ?: 0), $action, $module, (string)$targetId, (string)$ip, substr((string)$detailsJson, 0, 500)]));
            ...
            try {
                Database::query(
                    'INSERT INTO audit_logs (user_id, action, module, target_id, details_json, ip, user_agent)
                     VALUES (?, ?, ?, ?, ?, ?, ?)',
                    [Auth::id() ?: null, $action, $module, $targetId, $detailsJson, $ip, $userAgent]
                );
                return;
            } catch (Throwable) {
            }
    ...
    private static function writeFile(string $filename, string $message, array $context = []): void
    {
        try {
