...
$types = ReportBuilder::typesForRole('manager');
$projects = ManagerReport::projects($userId, $role);
$constituencies = ManagerReport::constituencies($userId, $role);
$recentRuns = ManagerReport::recentRuns($userId, 8);
$defaultFrom = date('Y-m-01');
$defaultTo = date('Y-m-d');

$pageTitle = 'Reports';
$pageDescription = 'Generate assigned-project progress, attendance, summary and register reports.';
$adminRole = 'manager';
$componentCss = ['cards', 'tables', 'reports'];
$pageScripts = ['report-builder'];
$contentClass = 'sa-reports-page manager-reports-page';
$csrfForm = 'reports';
$breadcrumbs = [
  ...
  <div class="reports-builder card">
    <div class="card__header">
      <div>
        ...
        <p class="card__subtitle">Choose report scope, filters and output format.</p>
      </div>
    </div>
...
<?php endforeach; ?>
      ...
      <div class="report-filter-grid">
        <label class="form-field"><span class="form-label">Date from</span><input class="form-input" type="date" name="date_from" value="<?= Security::e($defaultFrom) ?>"></label>
        <label class="form-field"><span class="form-label">Date to</span><input class="form-input" type="date" name="date_to" value="<?= Security::e($defaultTo) ?>"></label>
...
<?php endforeach; ?>
        ...
        <label class="form-field">
          ...
          <select class="form-select" name="constituency_id">
            <option value="">All assigned constituencies</option>
...
<?php endforeach; ?>
  ...
  <aside class="reports-recent card">
    <div class="card__header"><div><h2 class="card__title">Recent Runs</h2><p class="card__subtitle">Your latest generated reports.</p></div></div>
<?php if ($recentRuns === []): ?>
...
<?php foreach ($recentRuns as $run): ?>
      <div class="report-run">
        ...
        <div>
          ...
          <small><?= Security::e(strtoupper((string)$run['format'])) ?> / <?= Security::e(time_ago($run['created_at'])) ?></small>
        </div>
...
<section class="card report-preview-card" data-report-preview-card>
  <div class="card__header">
    <div><h2 class="card__title">Report Preview</h2><p class="card__subtitle">Preview output appears here before export or print.</p></div>
