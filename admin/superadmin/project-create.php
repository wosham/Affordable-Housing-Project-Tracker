...
$teamSelections = project_team_empty();
    ...
    if ($errors === []) {
        ...
        if (Project::slugExists($slug)) {
            ...
            try {
                ...
                save_project_geo_fence((int)$projectId, $old);
                sync_project_team((int)$projectId, $teamSelections, $teamUsers);
                save_project_milestones((int)$projectId, $_POST['milestones'] ?? []);
                Database::commit();

                Logger::log('create', 'projects', (int)$projectId, ['name' => $old['name']]);
                Session::flash('status', 'Project created successfully.');
                Response::redirect(Url::to('admin/superadmin/project-edit.php?id=' . (int)$projectId));
            } catch (Throwable $e) {
                Database::rollBack();
                $errors['form'] = 'Project could not be saved. Please check the fields and try again.';
                Logger::error('Project create failed', ['error' => $e->getMessage()]);
            }
        }
    }
}
...
$pageDescription = 'Create a new AHPTC project record.';
...
<section class="sa-projects-hero card">
  <div>
    <span class="sa-panel-label"><i class="fa-solid fa-plus" aria-hidden="true"></i> New Project</span>
    <h2>Create project record</h2>
    <p>Register a county housing site with delivery details, public imagery, contractor data and milestone tracking.</p>
  </div>
...
<form class="sa-project-form" method="post" action="<?= Security::e(Url::to('admin/superadmin/project-create.php')) ?>">
  <?= Csrf::field($csrfForm) ?>
  <div class="sa-editor-layout">
    <div class="sa-editor-main">
      ...
      <?php render_milestone_editor([]); ?>
      <div class="sa-form-actionbar">
        <span><i class="fa-solid fa-shield-halved" aria-hidden="true"></i> Secure CSRF-protected project create workflow</span>
        <div>
          <a class="btn btn--outline" href="<?= Security::e(Url::to('admin/superadmin/projects.php')) ?>">Cancel</a>
          <button class="btn btn--primary btn--lg" type="submit"><i class="fa-solid fa-floppy-disk" aria-hidden="true"></i> Create Project</button>
        </div>
      </div>
    </div>
    <aside class="sa-editor-aside" aria-label="Project preview">
      <?php render_project_preview($old, 'Create Preview'); ?>
    </aside>
  </div>
</form>
...
function save_project_geo_fence(int $projectId, array $data): void
{
    ...
    Database::query(
        "INSERT INTO geo_fences (project_id, site_name, latitude, longitude, radius_meters, status, created_by, verified_by, verified_at, notes)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)",
        [
            $projectId,
...
?>
  <section class="card sa-form-card">
    ...
    <div class="form-grid form-grid--2">
      <label class="form-field"> <span class="form-label">Project name</span>
        <input class="form-input <?= isset($errors['name']) ? 'is-error' : '' ?>" name="name" value="<?= Security::e($old['name']) ?>" required>
...
<?php foreach ($categories as $category): ?>
          <option value="<?= Security::e((string)$category['id']) ?>" <?= (string)$old['category_id'] === (string)$category['id'] ? 'selected' : '' ?>><?= Security::e($category['name']) ?></option>
<?php endforeach; ?>
        </select><?= field_error($errors, 'category_id') ?>
      </label>
      <label class="form-field"> <span class="form-label">Status</span>
        <select class="form-select <?= isset($errors['status']) ? 'is-error' : '' ?>" name="status">
<?php foreach (Project::statusOptions() as $status): ?>
          <option value="<?= Security::e($status) ?>" <?= $old['status'] === $status ? 'selected' : '' ?>><?= Security::e(status_label($status)) ?></option>
<?php endforeach; ?>
  ...
  <section class="card sa-form-card">
    ...
    <div class="form-grid form-grid--3">
      <label class="form-field"><span class="form-label">Constituency</span>
        <select class="form-select <?= isset($errors['constituency_id']) ? 'is-error' : '' ?>" name="constituency_id" data-project-constituency required>
          <option value="">Choose constituency</option>
<?php foreach ($constituencies as $constituency): ?>
          <option value="<?= Security::e((string)$constituency['id']) ?>" <?= (string)$old['constituency_id'] === (string)$constituency['id'] ? 'selected' : '' ?>><?= Security::e($constituency['name']) ?></option>
<?php endforeach; ?>
        </select><?= field_error($errors, 'constituency_id') ?>
      </label>
      <label class="form-field"><span class="form-label">Ward</span>
        <select class="form-select <?= isset($errors['ward_id']) ? 'is-error' : '' ?>" name="ward_id" data-project-ward data-selected-ward="<?= Security::e($old['ward_id']) ?>" <?= (int)$old['constituency_id'] <= 0 ? 'disabled' : '' ?>>
          <option value="">Choose constituency first</option>
<?php foreach ($wards as $ward): ?>
          <option value="<?= Security::e((string)$ward['id']) ?>" data-constituency-id="<?= Security::e((string)$ward['constituency_id']) ?>" <?= (string)$old['ward_id'] === (string)$ward['id'] ? 'selected' : '' ?>><?= Security::e($ward['name']) ?></option>
<?php endforeach; ?>
        </select><?= field_error($errors, 'ward_id') ?><span class="form-hint">Wards are filtered by the selected constituency.</span>
      </label>
      ...
      <label class="form-field"><span class="form-label">Start date</span><input class="form-input" type="date" name="start_date" value="<?= Security::e($old['start_date']) ?>"></label>
      <label class="form-field"><span class="form-label">Expected completion</span><input class="form-input" type="date" name="est_delivery" value="<?= Security::e($old['est_delivery']) ?>"></label>
...
{
    $rows = $milestones ?: array_fill(0, 5, ['label' => '', 'target_date' => '', 'actual_date' => '', 'status' => 'pending']);
?>
  <section class="card sa-form-card">
    <div class="card__header"><div><h2 class="card__title">Milestones</h2><p class="card__subtitle">Add the delivery steps shown on project timelines.</p></div></div>
...
<?php foreach ($rows as $index => $milestone): ?>
      <div class="sa-milestone-row">
        ...
        <label class="form-field"><span class="form-label">Status</span><select class="form-select" name="milestones[<?= (int)$index ?>][status]">
<?php foreach (['pending', 'current', 'done'] as $status): ?>
          <option value="<?= Security::e($status) ?>" <?= ($milestone['status'] ?? 'pending') === $status ? 'selected' : '' ?>><?= Security::e(status_label($status)) ?></option>
<?php endforeach; ?>
        </select></label>
      </div>
<?php endforeach; ?>
