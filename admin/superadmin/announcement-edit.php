...
<section class="card sa-announcement-hero">
  <div>
    <span class="sa-panel-label"><i class="fa-solid fa-pen-to-square" aria-hidden="true"></i> Edit Broadcast</span>
    <h2>Edit announcement</h2>
    <p>Update copy, schedule, priority, targeting and pinned status.</p>
  </div>
...
<section class="card sa-announcement-form-card">
  <div class="card__header">
    <div>
      ...
      <p class="card__subtitle">Originally created <?= Security::e(format_datetime($announcement['created_at'] ?? null)) ?> by <?= Security::e($announcement['author_name'] ?: 'Unknown author') ?>.</p>
    </div>
  ...
  <?php announcement_render_form($csrfForm, $values, Url::to('admin/superadmin/announcement-edit.php?id=' . $id), 'Save Changes', $id); ?>
</section>
...
function announcement_render_form(string $csrfForm, array $values, string $action, string $submitLabel, int $id): void
{
    ?>
    <form class="form-grid sa-announcement-form" method="post" action="<?= Security::e($action) ?>">
      <?= Csrf::field($csrfForm) ?>
      <input type="hidden" name="announcement_id" value="<?= Security::e((string)$id) ?>">
