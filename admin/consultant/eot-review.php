...
<section class="contract-hero card">
  <div>
    ...
    <h2>EOT Review</h2>
    <p>Assess extension requests, delay causes, supporting records and recommended days.</p>
  </div>
  <div class="contract-actions">
    ...
    <a class="btn btn--outline" href="<?= Security::e(Url::to('admin/consultant/variations.php')) ?>"><i class="fa-solid fa-code-branch"></i> Variations</a>
  </div>
</section>

<section class="contract-stats">
  <?php contract_stat('fa-file-circle-question', $summary['total'], 'EOT Requests', 'Assigned projects'); ?>
  ...
  <?php contract_stat('fa-calendar-days', format_number($summary['recommended_days']), 'Days Recommended', 'Current recommendation'); ?>
</section>
...
<section class="contract-layout">
  <article class="contract-card card">
    ...
    <div class="table-wrap">
      <table class="data-table contract-table">
        ...
        <tbody>
        <?php if ($items === []): ?>
          <tr><td colspan="7"><div class="contract-empty">No EOT requests found.</div></td></tr>
        <?php endif; ?>
  ...
  <aside class="contract-card card">
    ...
    <p>Requests waiting for consultant action.</p>
    <div class="contract-side-list">
      <?php foreach (array_slice(array_filter($items, static fn ($item): bool => (string)($item['consultant_review_status'] ?? 'pending') === 'pending'), 0, 7) as $item): ?>
        <div class="contract-side-item"><strong>EOT #<?= (int)$item['eot_number'] ?> / <?= Security::e($item['project_name']) ?></strong><span><?= format_number($item['days_requested']) ?> days requested</span></div>
      <?php endforeach; ?>
      <?php if ((int)$summary['pending_review'] <= 0): ?><div class="contract-empty">No pending EOT requests in this view.</div><?php endif; ?>
    </div>
  </aside>
</section>

<?php contract_modal('eot'); ?>
