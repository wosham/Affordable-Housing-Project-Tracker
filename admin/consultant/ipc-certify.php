...
if ($ipcId <= 0) {
    $candidate = ConsultantIPC::items(
        $userId,
        $role,
        ["status" => ConsultantIPC::CERTIFIABLE_STATUS],
        1,
        0,
    );
    if ($candidate === []) {
    ...
    if ($candidate !== []) {
        Response::redirect(
            Url::to(
                "admin/consultant/ipc-certify.php?id=" .
                    (int) $candidate[0]["id"],
            ),
        );
    }
...
$lineTotal = array_reduce(
    $lines,
    static fn(float $sum, array $line): float => $sum +
        (float) ($line["amount"] ?? 0),
    0.0,
);
$canCertify =
    (string) $ipc["status"] === ConsultantIPC::CERTIFIABLE_STATUS &&
    !$hasCriticalWarnings;
$canReject = in_array(
    (string) $ipc["status"],
    ["submitted", "clerk-endorsed"],
    true,
);

$pageTitle = "Certify IPC";
$pageDescription =
    "Review line items, warnings and certification history before taking action.";
$adminRole = "consultant";
$csrfForm = "consultant_ipc_certify";
...
$breadcrumbs = [
    ...
    [
        "label" => "Consultant",
        "url" => Url::to("admin/consultant/dashboard.php"),
    ],
    [
        "label" => "IPC Inbox",
        "url" => Url::to("admin/consultant/ipc-inbox.php"),
    ],
...
<section class="consultant-ipc-hero card">
  <div>
    ...
    <p><?= Security::e($ipc["project_name"]) ?> / <?= Security::e(
     $ipc["contractor_name"],
 ) ?> / <?= Security::e(format_date($ipc["period_from"])) ?> to <?= Security::e(
     format_date($ipc["period_to"]),
 ) ?></p>
  </div>
  <div class="consultant-ipc-hero__actions">
    <a class="btn btn--outline" href="<?= Security::e(
        Url::to("admin/consultant/ipc-inbox.php"),
    ) ?>"><i class="fa-solid fa-arrow-left" aria-hidden="true"></i> Inbox</a>
...
<section class="consultant-ipc-detail-grid">
  <article class="card consultant-ipc-certificate" data-consultant-ipc-print-area>
    <div class="card__header">
      ...
      <span class="badge <?= Security::e(
          status_badge_class($ipc["status"]),
      ) ?>"><?= Security::e(status_label($ipc["status"])) ?></span>
    </div>

    <div class="consultant-ipc-money-grid">
      <span><small>Gross</small><strong><?= Security::e(
          format_money($ipc["gross_amount"]),
      ) ?></strong></span>
      <span><small>Retention</small><strong><?= Security::e(
          format_money($ipc["retention_amount"]),
      ) ?></strong></span>
      <span><small>Net</small><strong><?= Security::e(
          format_money($ipc["net_amount"]),
      ) ?></strong></span>
      <span><small>Line Total</small><strong><?= Security::e(
          format_money($lineTotal),
      ) ?></strong></span>
      <span><small>Contract Sum</small><strong><?= Security::e(
          format_money($ipc["contract_sum"]),
      ) ?></strong></span>
      <span><small>Project Progress</small><strong><?= Security::e(
          format_number(percentage($ipc["pct_complete"] ?? 0)),
      ) ?>%</strong></span>
    </div>

    <div class="consultant-ipc-review-grid" aria-label="Certification review checks">
      <span><small>Review Lines</small><strong><?= Security::e(
          format_number($reviewMetrics["line_count"]),
      ) ?></strong></span>
      <span class="<?= (float) $reviewMetrics["gross_variance"] > 1
          ? "is-risk"
          : "" ?>"><small>Gross Variance</small><strong><?= Security::e(
    format_money($reviewMetrics["gross_variance"]),
) ?></strong></span>
      <span class="<?= (int) $reviewMetrics["over_boq"] > 0
          ? "is-risk"
          : "" ?>"><small>Over BOQ</small><strong><?= Security::e(
    format_number($reviewMetrics["over_boq"]),
) ?></strong></span>
      <span class="<?= (int) $reviewMetrics["warning_count"] > 0
          ? "is-risk"
          : "" ?>"><small>Warnings</small><strong><?= Security::e(
    format_number($reviewMetrics["warning_count"]),
) ?></strong></span>
    </div>

    <ol class="consultant-ipc-stepper">
<?php foreach (IPC::statusFlow() as $status => $step): ?>
      <li class="<?= (int) $ipc["workflow"]["current_step"] >=
      (int) $step["step"]
          ? "is-done"
          : "" ?>"><span><?= (int) $step["step"] ?></span><?= Security::e(
    $step["label"],
) ?></li>
<?php endforeach; ?>
    </ol>
    ...
    <div class="consultant-ipc-meta-grid">
      <span><small>Submitted</small><strong><?= Security::e(
          format_date($ipc["submitted_at"] ?? null),
      ) ?></strong></span>
      <span><small>Last action</small><strong><?= Security::e(
          status_label($ipc["last_action"] ?: $ipc["status"]),
      ) ?></strong></span>
      <span><small>Action by</small><strong><?= Security::e(
          $ipc["last_action_by"] ?: "System",
      ) ?></strong></span>
      <span><small>Ward</small><strong><?= Security::e(
          $ipc["ward_name"] ?: "-",
      ) ?></strong></span>
    </div>

<?php if ($warnings !== []): ?>
    <div class="consultant-ipc-warning-panel <?= $hasCriticalWarnings
        ? "is-critical"
        : "" ?>">
      <h3><i class="fa-solid fa-triangle-exclamation" aria-hidden="true"></i> Review warnings</h3>
      <ul><?php foreach ($warnings as $warning): ?><li><?= Security::e(
    $warning,
...
<?php else:foreach ($lines as $line): ?>
          <tr class="<?= !empty($line["over_boq"]) ||
          abs((float) ($line["amount_variance"] ?? 0)) > 1
              ? "is-warning"
              : "" ?>">
            <td><strong><?= Security::e(
                $line["item_no"] ?: "-",
            ) ?></strong><small><?= Security::e(
    $line["section"] ?: "General",
) ?></small></td>
            <td><?= Security::e(
                safe_truncate($line["description"] ?? "", 130),
            ) ?></td>
            <td class="is-num"><?= Security::e(
                format_number($line["previous_certified_qty"] ?? 0),
            ) ?></td>
            <td class="is-num"><?= Security::e(
                format_number($line["qty_this_period"]),
            ) ?></td>
            <td class="is-num"><?= Security::e(
                format_number($line["cumulative_qty"]),
            ) ?></td>
            <td class="is-num"><?= Security::e(
                format_number($line["boq_quantity"] ?? 0),
            ) ?></td>
            <td class="is-num"><?= Security::e(
                format_number($line["remaining_qty"] ?? 0),
            ) ?></td>
            <td class="is-num"><?= Security::e(
                format_money($line["rate"]),
            ) ?></td>
            <td class="is-num"><strong><?= Security::e(
                format_money($line["amount"]),
            ) ?></strong><?php if (
    abs((float) ($line["amount_variance"] ?? 0)) > 1
): ?><small class="consultant-ipc-warning">Variance <?= Security::e(
    format_money($line["amount_variance"]),
) ?></small><?php endif; ?></td>
          </tr>
  ...
  <aside class="consultant-ipc-action-stack">
    <article class="card consultant-ipc-action-card">
      ...
      <form data-consultant-ipc-certify-form>
        ...
        <label class="consultant-ipc-check"><input type="checkbox" name="checklist[]" value="<?= Security::e(
            $value,
        ) ?>" required> <?= Security::e($label) ?></label>
        <?php endforeach; ?>
        ...
        <?php if (
            (string) $ipc["status"] !== ConsultantIPC::CERTIFIABLE_STATUS
        ): ?><p class="consultant-ipc-critical-note">This IPC is currently <?= Security::e(
    status_label($ipc["status"]),
) ?> and cannot be certified from this step.</p><?php endif; ?>
        <?php if (
            $hasCriticalWarnings
        ): ?><p class="consultant-ipc-critical-note">Resolve critical warnings before certifying this IPC.</p><?php endif; ?>
        <div class="consultant-ipc-action-buttons">
          <button class="btn btn--success" type="submit" <?= $canCertify
              ? ""
              : "disabled" ?>><i class="fa-solid fa-stamp" aria-hidden="true"></i> Certify IPC</button>
          <?php if (
              $canReject
          ): ?><button class="btn btn--danger" type="button" data-consultant-ipc-reject data-ipc-id="<?= (int) $ipc[
    "id"
] ?>" data-ipc-label="IPC #<?= Security::e(
    format_number($ipc["ipc_number"]),
) ?>"><i class="fa-solid fa-ban" aria-hidden="true"></i> Reject</button><?php endif; ?>
        </div>
      </form>
    </article>
...
<?php else:foreach ($history as $entry): ?>
        <div class="consultant-ipc-history__item">
          ...
          <div><strong><?= Security::e(
              status_label($entry["action"]),
          ) ?></strong><small><?= Security::e(
    $entry["actor_name"],
) ?> / <?= Security::e($entry["actor_role"] ?: "Staff") ?> / <?= Security::e(
     format_date($entry["actioned_at"]),
 ) ?></small><?php if (!empty($entry["comments"])): ?><p><?= Security::e(
    $entry["comments"],
) ?></p><?php endif; ?></div>
        </div>
<?php endforeach;endif; ?>
...
<div class="consultant-ipc-modal" data-consultant-ipc-modal hidden>
  <form class="consultant-ipc-modal__panel" data-consultant-ipc-reject-form>
    ...
    <div class="consultant-ipc-modal__body">
      <input type="hidden" name="ipc_id" data-consultant-ipc-reject-id value="<?= (int) $ipc[
          "id"
      ] ?>">
      <p data-consultant-ipc-reject-label>Record why IPC #<?= Security::e(
          format_number($ipc["ipc_number"]),
      ) ?> is being returned.</p>
