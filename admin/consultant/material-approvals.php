...
<section class="technical-hero card">
  <div>
    ...
    <p>Review material submissions against specifications and keep approval decisions visible to the project team.</p>
  </div>
  <div class="technical-actions">
    ...
    <a class="btn btn--outline" href="<?= Security::e(Url::to('admin/consultant/shop-drawings.php')) ?>"><i class="fa-solid fa-compass-drafting"></i> Shop Drawings</a>
  </div>
</section>

<section class="technical-stats">
  <?php technical_stat('fa-boxes-stacked', $summary['total'], 'Submissions', 'Assigned projects'); ?>
  <?php technical_stat('fa-hourglass-half', $summary['pending'], 'Pending', 'Awaiting review'); ?>
  <?php technical_stat('fa-circle-check', $summary['approved'], 'Approved', 'Accepted materials'); ?>
  <?php technical_stat('fa-rotate-left', $summary['rejected'], 'Returned', 'Needs correction'); ?>
  <?php technical_stat('fa-calendar-week', $summary['new_week'], 'This Week', 'Recent submissions'); ?>
</section>
...
<section class="technical-layout">
  <article class="technical-card card">
    ...
    <div class="table-wrap">
      <table class="data-table technical-table">
        ...
        <tbody>
        <?php if ($items === []): ?>
          <tr><td colspan="7"><div class="technical-empty">No material submissions found.</div></td></tr>
        <?php endif; ?>
  ...
  <aside class="technical-card card">
    ...
    <p>Recent submissions waiting for a decision.</p>
    <div class="technical-side-list">
      <?php foreach (array_slice(array_filter($items, static fn ($item): bool => (string)$item['status'] === 'pending'), 0, 7) as $item): ?>
        <div class="technical-side-item"><strong><?= Security::e($item['material']) ?></strong><span><?= Security::e($item['project_name']) ?> / <?= Security::e(format_date($item['submitted_date'])) ?></span></div>
      <?php endforeach; ?>
      <?php if ((int)$summary['pending'] <= 0): ?><div class="technical-empty">No pending material submissions in this view.</div><?php endif; ?>
    </div>
  </aside>
</section>
