...
$summary = ConsultantTechnicalReview::materialSummary($userId, $role, $filters);
$items = ConsultantTechnicalReview::materialItems($userId, $role, $filters, $limit, $offset);
$total = ConsultantTechnicalReview::materialCount($userId, $role, $filters);
$pages = max(1, (int)ceil($total / $limit));
$pendingItems = ConsultantTechnicalReview::materialPendingItems($userId, $role, $filters, 8);

$pageTitle = 'Material Approvals';
$pageDescription = 'Review submitted materials, specifications and approval outcomes.';
...
<section class="technical-hero card">
  <div>
    <span class="sa-panel-label"><i class="fa-solid fa-cubes-stacked" aria-hidden="true"></i> Material review</span>
    <h2>Material Approvals</h2>
    <p>Review material submissions against specifications and keep approval decisions visible to the project team on assigned projects only.</p>
  </div>
  <div class="technical-actions">
    ...
    <a class="btn btn--outline" href="<?= Security::e(Url::to('admin/consultant/shop-drawings.php')) ?>"><i class="fa-solid fa-compass-drafting" aria-hidden="true"></i> Shop Drawings</a>
  </div>
</section>

<section class="technical-stats" aria-label="Material summary">
  <?php technical_stat('fa-boxes-stacked', $summary['total'], 'Submissions', 'Assigned projects', material_filter_url([])); ?>
  <?php technical_stat('fa-hourglass-half', $summary['pending'], 'Pending', 'Awaiting review', material_filter_url(['status' => 'pending'])); ?>
  <?php technical_stat('fa-circle-check', $summary['approved'], 'Approved', 'Accepted materials', material_filter_url(['status' => 'approved'])); ?>
  <?php technical_stat('fa-rotate-left', $summary['rejected'], 'Returned', 'Needs correction', material_filter_url(['status' => 'rejected'])); ?>
  <?php technical_stat('fa-calendar-week', $summary['new_week'], 'This Week', 'Recent submissions', material_filter_url([])); ?>
</section>
...
<section class="technical-layout">
  <article class="technical-card card">
    <div class="technical-card__head">
      <div>
        ...
        <p>Approve suitable materials or return submissions with clear correction notes.</p>
      </div>
      <span class="badge badge--info"><?= format_number($total) ?> records</span>
    </div>

    <form class="technical-filter-grid" method="get" action="<?= Security::e(Url::to('admin/consultant/material-approvals.php')) ?>">
      <label>Search <input type="search" name="q" value="<?= Security::e($filters['q']) ?>" placeholder="Material, spec or project..."></label>
      <label>Project
      ...
      <a class="btn btn--outline" href="<?= Security::e(Url::to('admin/consultant/material-approvals.php')) ?>">Reset</a>
    </form>

    <div class="table-wrap">
      <table class="data-table technical-table">
        <thead><tr><th>Material</th><th>Project</th><th>Specification</th><th>Submitted</th><th>Status</th><th>Decision</th><th>Actions</th></tr></thead>
        <tbody>
        <?php if ($items === []): ?>
          <tr>
            <td colspan="7">
              <div class="empty-state">
                <strong class="empty-state__title">No material submissions found</strong>
                <span class="empty-state__text">Adjust filters or wait for material submissions on assigned projects.</span>
              </div>
            </td>
          </tr>
        <?php endif; ?>
        <?php foreach ($items as $item): ?>
          <tr>
            <td>
              <span class="technical-primary"><?= Security::e($item['material']) ?></span>
              <span class="technical-secondary">Submitted by <?= Security::e($item['submitted_by_name'] ?: 'Project team') ?></span>
            </td>
            <td>
            ...
            <td><?= Security::e(safe_truncate($item['specification'] ?? '', 140)) ?></td>
            <td><?= Security::e(format_date($item['submitted_date'])) ?></td>
            ...
            <td>
              <span><?= Security::e($item['approved_by_name'] ?: '-') ?></span>
              <span class="technical-secondary"><?= Security::e(format_date($item['approved_date'] ?? null)) ?></span>
              <span class="technical-secondary"><?= Security::e($item['notes'] ? safe_truncate($item['notes'], 70) : '') ?></span>
            </td>
            <td><div class="technical-row-actions">
              <?php technical_action('material', (int)$item['id'], 'approve', 'Approve', 'fa-check', $item['material'] . ' - ' . $item['project_name']); ?>
              <?php technical_action('material', (int)$item['id'], 'return', 'Return', 'fa-rotate-left', $item['material'] . ' - ' . $item['project_name']); ?>
            </div></td>
          </tr>
        <?php endforeach; ?>
        </tbody>
  ...
  <aside class="technical-card card">
    ...
    <p>Submissions waiting for a decision across your portfolio (not limited to this page).</p>
    <div class="technical-side-list">
      <?php if ($pendingItems === []): ?>
        <div class="empty-state empty-state--compact">
          <strong class="empty-state__title">No pending materials</strong>
          <span class="empty-state__text">New submissions will appear here.</span>
        </div>
      <?php else: foreach ($pendingItems as $item): ?>
        <a class="technical-side-item technical-side-item--link" href="<?= Security::e(Url::to('admin/consultant/material-approvals.php?project_id=' . (int)$item['project_id'] . '&status=pending&q=' . rawurlencode((string)$item['material']))) ?>">
          <strong><?= Security::e($item['material']) ?></strong>
          <span><?= Security::e($item['project_name']) ?> · <?= Security::e(format_date($item['submitted_date'])) ?></span>
        </a>
      <?php endforeach; endif; ?>
    </div>
    <?php if ((int)$summary['pending'] > 0): ?>
      <a class="btn btn--outline btn--sm" href="<?= Security::e(material_filter_url(['status' => 'pending'])) ?>">View pending only</a>
    <?php endif; ?>
  </aside>
</section>
...
function material_filter_url(array $extra): string
{
    $query = array_filter(array_merge($_GET, $extra), static fn ($v) => $v !== '' && $v !== null && $v !== 0 && $v !== '0');
    unset($query['page']);
    return Url::to('admin/consultant/material-approvals.php' . ($query ? '?' . http_build_query($query) : ''));
}
