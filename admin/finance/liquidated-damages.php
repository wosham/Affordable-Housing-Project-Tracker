...
$total = FinanceBudget::liquidatedDamageCount($filters);
$totalPages = max(1, (int)ceil($total / $perPage));
$page = min($page, $totalPages);
$offset = ($page - 1) * $perPage;
$rows = FinanceBudget::liquidatedDamageRows($filters, $perPage, $offset);
...
$pageDescription = 'Track assessed damages, status and linked payment certificates.';
$adminRole = 'finance';
$csrfForm = 'finance_reports';
$contentClass = 'finance-payments-page finance-ld-page';
...
<section class="finance-hero card">
  <div>
    <span class="sa-panel-label"><i class="fa-solid fa-money-bill-trend-up" aria-hidden="true"></i> Contract controls</span>
    <h2>Liquidated Damages</h2>
    <p>Review assessed damages, application status and linked IPC deductions. This register is read-only for finance review.</p>
  </div>
  <div class="finance-hero__actions">
    ...
    <a class="btn btn--primary" href="<?= Security::e(Url::to('api/finance/generate-report.php?type=ld_report&format=csv&' . http_build_query($filters))) ?>"><i class="fa-solid fa-file-csv" aria-hidden="true"></i> Export CSV</a>
  </div>
</section>

<section class="finance-stat-grid finance-stat-grid--3" aria-label="Damages summary">
  <?php finance_ld_stat('fa-triangle-exclamation', format_money($stats['ld_value']), 'Damages Value', 'Active exposure', 'admin/finance/liquidated-damages.php'); ?>
  <?php finance_ld_stat('fa-building', $stats['projects'], 'Projects', 'Current filter', 'admin/finance/budget-tracker.php'); ?>
  <?php finance_ld_stat('fa-list', format_number($total), 'Records', 'Matching filter', 'admin/finance/liquidated-damages.php'); ?>
</section>
...
<nav class="finance-hub card" aria-label="Finance modules">
  ...
  <a class="finance-hub__link is-active" href="<?= Security::e(Url::to('admin/finance/liquidated-damages.php')) ?>"><i class="fa-solid fa-money-bill-trend-up"></i><span>Damages</span></a>
...
<section class="card finance-card finance-card--full">
  <div class="card__header">
    <div>
      <h2 class="card__title">Damages Register</h2>
      <p class="card__subtitle">Financial impact and application status by project.</p>
    </div>
    <span class="badge badge--lime"><?= Security::e(format_number($total)) ?> records</span>
  </div>

  <form class="finance-filter finance-filter--wide" method="get" action="<?= Security::e(Url::to('admin/finance/liquidated-damages.php')) ?>">
    ...
    <label class="filter-group" for="status"><span class="filter-label">Status</span><select class="form-select" id="status" name="status"><option value="">All statuses</option><?php foreach (['draft', 'pending', 'applied', 'suspended', 'waived'] as $status): ?><option value="<?= $status ?>" <?= (($filters['status'] ?? '') === $status) ? 'selected' : '' ?>><?= Security::e(status_label($status)) ?></option><?php endforeach; ?></select></label>
    ...
    <div class="filter-actions"><button class="btn btn--primary" type="submit"><i class="fa-solid fa-filter" aria-hidden="true"></i> Filter</button><a class="btn btn--outline" href="<?= Security::e(Url::to('admin/finance/liquidated-damages.php')) ?>">Reset</a></div>
  </form>
    ...
    <table class="data-table finance-table finance-table--stack">
      <thead>
        <tr>
          ...
          <th>Status</th>
          <th>Days</th>
          <th class="is-money">Daily Rate</th>
      ...
      <tbody>
<?php if ($rows === []): ?>
        <tr><td colspan="8"><div class="empty-state"><strong class="empty-state__title">No liquidated damages found</strong><span class="empty-state__text">Assessed damages will appear here when recorded against projects.</span></div></td></tr>
<?php else: foreach ($rows as $row): ?>
        <tr>
          <td data-label="Project"><strong><?= Security::e($row['project_name']) ?></strong></td>
          <td data-label="Status"><span class="badge <?= Security::e($row['status_badge']) ?>"><?= Security::e(status_label($row['status'] ?? 'draft')) ?></span></td>
          <td data-label="Days"><?= Security::e(format_number((int)$row['days_overdue'])) ?></td>
          <td class="is-money" data-label="Daily Rate"><?= Security::e(format_money((float)$row['rate_per_day'])) ?></td>
          <td class="is-money" data-label="Total"><strong><?= Security::e(format_money((float)$row['total_ld'])) ?></strong></td>
          <td data-label="Linked IPC"><?= !empty($row['ipc_number']) ? 'IPC #' . Security::e($row['ipc_number']) : '-' ?></td>
          <td data-label="Calculated By"><?= Security::e($row['calculated_by_name'] ?: '-') ?></td>
          <td data-label="Updated"><?= Security::e(format_datetime($row['updated_at'] ?? null)) ?></td>
        </tr>
<?php endforeach; endif; ?>
  ...
  <nav class="pagination" aria-label="LD pagination">
    ...
    <div class="pagination__links">
      <a class="pagination__link<?= $page <= 1 ? ' is-disabled' : '' ?>" href="<?= Security::e(finance_ld_page_url($filters, max(1, $page - 1))) ?>"><i class="fa-solid fa-chevron-left" aria-hidden="true"></i></a>
      <span class="pagination__link is-active"><?= Security::e(format_number($page)) ?> / <?= Security::e(format_number($totalPages)) ?></span>
      <a class="pagination__link<?= $page >= $totalPages ? ' is-disabled' : '' ?>" href="<?= Security::e(finance_ld_page_url($filters, min($totalPages, $page + 1))) ?>"><i class="fa-solid fa-chevron-right" aria-hidden="true"></i></a>
    </div>
  </nav>
</section>
...
<?php
function finance_ld_stat(string $icon, mixed $value, string $label, string $hint, string $href = ''): void
{
...
function finance_ld_page_url(array $filters, int $page): string
{
    $query = array_merge($filters, ['page' => $page]);
    $query = array_filter($query, static fn ($value): bool => $value !== null && $value !== '' && $value !== 0);
    return Url::to('admin/finance/liquidated-damages.php' . ($query ? '?' . http_build_query($query) : ''));
}
