...
if ($type === '' || $id <= 0 || $action === '') {
    Response::json(['success' => false, 'message' => 'Quality record and action are required.'], 422);
}

try {
    $result = ConsultantQuality::applyAction($type, $id, $action, $input, (int)Auth::id(), (string)Auth::role());
