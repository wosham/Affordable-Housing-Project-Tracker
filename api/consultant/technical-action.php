...
if (!consultant_technical_csrf_ok()) {
...
if ($type === '' || $id <= 0 || $action === '') {
    Response::json(['success' => false, 'message' => 'Review type, item and action are required.'], 422);
}

try {
    $result = ConsultantTechnicalReview::applyAction($type, $id, $action, $note, (int)Auth::id(), (string)Auth::role());
...
function consultant_technical_csrf_ok(): bool
{
    return ApiCsrf::checkAny(['consultant_technical', 'consultant_ipcs', 'consultant_ipc_inbox', 'default']);
}
